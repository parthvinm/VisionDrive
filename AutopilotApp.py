...
model = load_model('/Users/parthvinm/Desktop/DL/Autopilot/Autopilot_V2/models/Autopilot_new.h5')
...
cap = cv2.VideoCapture('/Users/parthvinm/Desktop/DL/Autopilot/Autopilot_V2/resources/run.mp4', cv2.CAP_AVFOUNDATION)
while (cap.isOpened()):
    ret, frame = cap.read()
    if not ret: 
        break
    ...
    results = results.xyxy[0].cpu().numpy()  # Convert detection results to NumPy array

    # Filter detections by class ID and draw bounding boxes
    for *box, conf, cls_id in results:
        if cls_id in CLASS_ID:
            x1, y1, x2, y2 = map(int, box)
            cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
            label = f"{CLASS_NAMES_DICT[int(cls_id)]} {conf:.2f}"
